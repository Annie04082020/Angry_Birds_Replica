...
- **正常模式與作弊模式**
  - 在關卡選擇畫面中設計了「正常模式」與「作弊模式」兩種切換機制，主要用於控制關卡的解鎖規則與測試流程。系統會讀取 unlock_progress.json 中紀錄的最高連續通關進度，並依此決定玩家目前可遊玩的關卡範圍。
  - 在**正常模式（Normal Mode）**下，玩家只能選擇目前已解鎖的關卡。系統會以「最高連續通關關卡 + 1」作為可開啟的最新關卡，例如若玩家已連續通關到第 3 關，則最多只能進入第 4 關。這種設計符合一般遊戲的循序解鎖邏輯，能夠保留關卡推進感與挑戰性，同時也會在每次過關後自動更新存檔進度。
  - 在**作弊模式（Cheat Mode）**下，關卡選擇畫面會暫時解除解鎖限制，讓玩家可以直接進入任意一個已實作的關卡，不受通關進度約束。此模式可透過 F10 鍵在選單場景中切換，主要用途是方便開發期間進行關卡測試、除錯與展示，避免每次都必須從前面關卡重新遊玩。當模式切換後，關卡選擇畫面上的提示文字也會同步更新，讓玩家清楚知道目前所處的模式。
  - 另外，作弊模式僅影響關卡是否可被選取，不會改動遊戲中的物理系統、分數計算或角色能力數值，因此能在不破壞核心玩法的前提下，提升開發與測試效率。
